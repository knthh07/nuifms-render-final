...
const CampusManagement = () => {
    ...
    const fetchCampuses = async () => {
        try {
            ...
            setSnackbar({ open: true, message: 'Failed to load campuses', severity: 'error' });
            setCampuses([]);
        }
    };
    ...
    const handleOpenFloorDialog = (buildingId) => {
        const campus = campuses.find(c => c.buildings.some(b => b._id === buildingId));
        if (campus) {
    ...
    const handleCampusSubmit = async (e) => {
        e.preventDefault();
        if (!campusName) {
            setSnackbar({ open: true, message: 'Campus name is required', severity: 'warning' });
            return;
        }

        try {
            if (editingCampus) {
                await axios.put(`/api/campuses/${editingCampus._id}`, { name: campusName });
            } else {
            ...
            setSnackbar({ open: true, message: 'Failed to save campus', severity: 'error' });
        }
    };
    ...
    const handleBuildingSubmit = async (e) => {
        e.preventDefault();
        if (!buildingName) {
            setSnackbar({ open: true, message: 'Building name is required', severity: 'warning' });
            return;
        }

        try {
            await axios.post(`/api/campuses/${selectedCampusId}/buildings`, { name: buildingName });
            fetchCampuses();
            handleCloseDialog();
        } catch (error) {
            console.error('Error saving building:', error);
            setSnackbar({ open: true, message: 'Failed to save building', severity: 'error' });
        }
    };
    ...
    const handleFloorSubmit = async (e) => {
        e.preventDefault();
        if (!floorName) {
            setSnackbar({ open: true, message: 'Floor number is required', severity: 'warning' });
            return;
        }

        try {
            await axios.post(`/api/campuses/${selectedCampusId}/buildings/${selectedBuildingId}/floors`, { number: floorName });
            fetchCampuses();
            handleCloseDialog();
        } catch (error) {
            console.error('Error saving floor:', error);
            setSnackbar({ open: true, message: 'Failed to save floor', severity: 'error' });
        }
    };
    ...
    const handleOfficeSubmit = async (e) => {
        ...
        if (!selectedCampusId || !selectedBuildingId || !selectedFloorId) {
            console.error('Missing IDs:', { selectedCampusId, selectedBuildingId, selectedFloorId });
            alert('Unable to add office: Campus, Building, or Floor ID is missing.');
            return;
        }

        try {
            console.log('Creating office with IDs:', { selectedCampusId, selectedBuildingId, selectedFloorId });
            ...
            console.error('Error saving office:', error.response || error.message || error);
            alert('Failed to save office. Please try again.');
        }
    };
    ...
    const handleDeleteCampus = async (campusId) => {
        if (window.confirm('Are you sure you want to delete this campus?')) {
            try {
                ...
                setSnackbar({ open: true, message: 'Failed to delete campus', severity: 'error' });
            }
        }
    };
    ...
    return (
        <LayoutComponent>
            ...
            <div className=' w-full p-4'>
                ...
                <Button variant="contained" color="primary" onClick={() => handleOpenCampusDialog()}>
                    Add Campus
                </Button>
                            ...
                            {campuses.map((campus) => (
                                <React.Fragment key={campus._id}>
                                    <TableRow>
                                        ...
                                        <TableCell>
                                            <Button onClick={() => handleOpenCampusDialog(campus)}>Edit</Button>
                                            <Button onClick={() => handleOpenBuildingDialog(campus._id)}>Add Building</Button>
                                            <Button onClick={() => handleDeleteCampus(campus._id)} color="secondary">Delete</Button>
                                                            ...
                                                            <TableRow key={building._id}>
                                                                ...
                                                                <TableCell>
                                                                    {building.floors?.map((floor) => (
                                                                        <div key={floor._id}>
                                                                            <Typography variant="body2">Floor: {floor.number}</Typography>
                                                                            <Button onClick={() => handleOpenOfficeDialog(floor._id)}>Add Office</Button>
                                                                ...
                                                                <TableCell>
                                                                    <Button onClick={() => handleOpenFloorDialog(building._id)}>Add Floor</Button>
                                                                </TableCell>
                                                            </TableRow>
                                                        ))}
                                                    </TableBody>
                ...
                <Dialog open={openBuildingDialog} onClose={handleCloseDialog}>
                    ...
                    <DialogActions>
                        <Button onClick={handleCloseDialog}>Cancel</Button>
                        <Button onClick={handleBuildingSubmit} color="primary">Save</Button>
                    </DialogActions>
                </Dialog>
                ...
                <Dialog open={openFloorDialog} onClose={handleCloseDialog}>
                    ...
                    <DialogActions>
                        <Button onClick={handleCloseDialog}>Cancel</Button>
                        <Button onClick={handleFloorSubmit} color="primary">Save</Button>
                    </DialogActions>
                </Dialog>
                ...
                <Dialog open={openOfficeDialog} onClose={handleCloseDialog}>
                    ...
                    <DialogActions>
                        <Button onClick={handleCloseDialog}>Cancel</Button>
                        <Button onClick={handleOfficeSubmit} color="primary">Save</Button>
                    </DialogActions>
                </Dialog>
