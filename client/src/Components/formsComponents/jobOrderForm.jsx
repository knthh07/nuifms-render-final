...
const scenarioToObjects = {
    ...
    Slippery: {
        severity: "Minor",
        objects: ["Floor", "Stairs", "Entrance", "Bathroom tiles", "Balcony"],
    },
    Leaking: {
        severity: "Critical",
        objects: ["Faucet", "Pipes", "Roof", "Water dispenser", "Sink", "Ceiling"],
    },
    Clogged: {
        severity: "Minor",
        objects: ["Toilet", "Drain", "Sink", "Gutter", "AC Vent"],
    },
    Noisy: {
        severity: "Minor",
        objects: ["Fan", "Door", "Ventilation system", "Generator", "AC unit"],
    },
    "Not Working": {
        severity: "Critical",
        objects: [
    ...
    Cracked: {
        severity: "Moderate",
        objects: ["Window", "Door", "Floor tile", "Wall", "Whiteboard"],
    },
    "Burnt Out": {
        severity: "Moderate",
        objects: [
...
const JobOrderForm = () => {
    ...
    const handleCampusChange = useCallback(
        (e) => {
            ...
            const selectedCampus = data.find(
                (campus) => campus.name === selectedCampusName
            ); // Find the campus object
    ...
    const handleBuildingChange = useCallback(
        (e) => {
            ...
            const selectedCampusData = data.find(
                (campus) => campus.name === jobOrder.campus
            ); // Find campus by name
            if (selectedCampusData) {
                const selectedBuildingData = selectedCampusData.buildings.find(
    ...
    const handleFloorChange = useCallback(
        (e) => {
            ...
            if (selectedCampusData) {
                const selectedBuildingData = selectedCampusData.buildings.find(
                    (b) => b.name === jobOrder.building
                );
                if (selectedBuildingData) {
                    const selectedFloorData = selectedBuildingData.floors.find(
                        (f) => f.number === selectedFloorName
                    );
    ...
    const submitJobOrder = useCallback(
        async (e) => {
            e.preventDefault();

            // Determine the requesting office to submit
            const reqOfficeToSubmit =
                jobOrder.reqOffice === "Other"
                    ? jobOrder.otherReqOffice
                    : jobOrder.reqOffice;

            // Determine scenario and object to submit
            const scenarioToSubmit =
                jobOrder.scenario === "Other" ? otherScenario : jobOrder.scenario;
            const objectToSubmit =
                jobOrder.object === "Other" ? otherObject : jobOrder.object;
        ...
        [jobOrder]
    ); // Add otherScenario and otherObject to the dependency array
                    ...
                    <div className="flex">
                        <div className="flex-wrap justify-between p-4 y-4 w-full">
                            <TextField
                                ...
                                onChange={(e) =>
                                    setJobOrder({ ...jobOrder, jobType: e.target.value })
                                }
                            ...
                            <Tooltip
                                title="Please select a campus first."
                                arrow
                                disableHoverListener={!!jobOrder.campus}
                            >
                                <Box mb={2}>
                                    ...
                                    <Box display="flex" gap={2} mb={2}>
                                        ...
                                        >
                                            {data
                                                .find((campus) => campus.name === jobOrder.campus)
                                                ?.buildings.map((building) => (
                                                    <MenuItem key={building._id} value={building.name}>
                                                        {" "}
                                                        {/* Change value to building.name */}
                                                        {building.name}
                                                    </MenuItem>
                                                )) || (
                                        ...
                                        >
                                            {floors.map((floor) => (
                                                <MenuItem key={floor._id} value={floor.number}>
                                                    {" "}
                                                    {/* Change value to floor.number */}
                                                    {floor.number}{" "}
                                                    {/* Assuming `number` is the floor name */}
                                                </MenuItem>
                        ...
                        /> */}
                            ...
                            <Tooltip
                                ...
                                arrow
                                disableHoverListener={!jobOrder.scenario}
                            >
                                    ...
                                    <TextField
                                        ...
                                        onChange={(e) => {
                                            ...
                                            setJobOrder({
                                                ...
                                                scenario: selectedScenario,
                                                object: "",
                                            }); // Clear object if scenario changes
                                            ...
                                            setObjects(
                                                selectedScenario === "Other"
                                                    ? []
                                                    : scenarioToObjects[selectedScenario]?.objects || []
                                            );
                                        }}
                                        ...
                                        {sortedScenarios.map(([scenario, { severity }]) => (
                                            <MenuItem
                                                ...
                                                sx={{
                                                    backgroundColor:
                                                        severity === "Critical"
                                                            ? "#ffcccc" // Light red for Critical
                                                            : severity === "Moderate"
                                                                ? "#ffffcc" // Light yellow for Moderate
                                                                : "#ccffcc", // Light green for Minor
                                                    "&:hover": {
                                                        backgroundColor:
                                                            severity === "Critical"
                                                                ? "#ff9999" // Darker red on hover
                                                                : severity === "Moderate"
                                                                    ? "#ffff99" // Darker yellow on hover
                            ...
                            <Tooltip
                                ...
                                arrow
                                disableHoverListener={!jobOrder.object}
                            >
                                <Box display="flex" gap={2} mb={2}>
                                    <TextField
                                        id="object"
                            ...
                            <Box>
                                <TextField
                                    ...
                                    onChange={(e) =>
                                        setJobOrder({ ...jobOrder, jobDesc: e.target.value })
                                    }
                                ...
                                {charactersLeft < 0 && (
                                    <FormHelperText error>
                                        You have exceeded the character limit by {-charactersLeft}{" "}
                                        characters.
                                    </FormHelperText>
                                )}
                            </Box>
